# GovTech Services

Die GovTech Services von admin.digital unterstützen Gemeinden, Kantone und NGO bei der digitalen Transformation.
Jeder Service basiert auf modular aufgebauten GovTech [Modulen](/module/).


<div class="grid cards" markdown>

-   :material-desktop-classic:{ .lg .middle } **Gemeindeportal**

	---

	Digitales Portal für Gemeinden.

	[:octicons-arrow-right-24: Mehr erfahren](/services/gemeindeportal/)

-   :material-format-list-bulleted:{ .lg .middle } **Publikationen für Wahlen und Abstimmungen**

	---

	Publikationen und Informationen rund um Wahlen und Abstimmungen.

	[:octicons-arrow-right-24: Mehr erfahren](/services/publikationen-wahlen-abstimmungen/)

-   :material-calendar-month-outline:{ .lg .middle } **Buchungsplattform Ferienangebote (Pro Juventute)**

	---

	Buchungsplattform für Ferienangebote.

	[:octicons-arrow-right-24: Mehr erfahren](/services/feriennet/)

-   :material-account-multiple:{ .lg .middle } **Koordination Dolmetschdienst**

	---

	Koordination von Dolmetschenden und Einsätzen.

	[:octicons-arrow-right-24: Mehr erfahren](/services/dolmetschkoordination/)

-   :material-calendar-outline:{ .lg .middle } **Staatskalender**

	---

	Publikation und Verwaltung von Personen und Organisationen im Staatskalender.

	[:octicons-arrow-right-24: Mehr erfahren](/services/staatskalender/)

-   :material-school-outline:{ .lg .middle } **Kursverwaltung**

	---

	Verwaltung von Kursen, Terminen und Anmeldungen.

	[:octicons-arrow-right-24: Mehr erfahren](/services/kursverwaltung/)

-   :material-file-document-outline:{ .lg .middle } **Planauflage Baugesuche**

	---

	Digitale Publikation von Unterlagen zur Planauflage von Baugesuchen.

	[:octicons-arrow-right-24: Mehr erfahren](/services/planauflage-baugesuche/)

-   :material-account-group-outline:{ .lg .middle } **Versammlungen (Landsgemeinde)**

	---

	Organisation und Publikation von Versammlungen – inkl. Informationen und Unterlagen.

	[:octicons-arrow-right-24: Mehr erfahren](/services/versammlungen-landsgemeinde/)

-   :material-database-outline:{ .lg .middle } **Datenportal Abstimmungen (Swissvotes)**

	---

	Bereitstellung und Auswertung von Abstimmungsdaten.

	[:octicons-arrow-right-24: Mehr erfahren](/services/datenportal-abstimmungen-swissvotes/)

-   :material-domain:{ .lg .middle } **Intranet**

	---

	Interne Plattform für Informationen, Dokumente und Zusammenarbeit.

	[:octicons-arrow-right-24: Mehr erfahren](/services/intranet/)

</div>
